...
type AgentLoopParams = {
  ...
  provider: any; // Todo change any to the correct type
};
...
export class AgentLoop {
  ...
  private provider: any; // Todo change any to the correct type
  ...
  public async run(
    ...
    try {
      ...
      while (turnInput.length > 0) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            stream = await this.provider.createStream({
              model: this.model,
                ...
                content: [
                  {
                    ...
                    text: (() => {
                      ...
                      return `⚠️  LLM rejected the request${
      ...
      if (isNetworkOrServerError) {
        try {
          const msgText =
            "⚠️  Network error while contacting LLM provider. Please check your connection and try again.";
          this.onItem({
            id: `error-${Date.now()}`,
            type: "message",
            role: "system",
      ...
      if (isInvalidRequestError()) {
        try {
          ...
          const msgText = `⚠️  LLM provider rejected the request${
            reqId ? ` (request ID: ${reqId})` : ""
          }. Error details: ${errorDetails}. Please verify your settings and try again.`;
