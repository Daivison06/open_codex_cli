...
import { generateChatCompletion } from "../../utils/llm-provider";
...
/**
 * Generates an explanation for a shell command using the LLM provider.
 *
...
async function generateCommandExplanation(
  ...
  try {
    ...
    const explanation = await generateChatCompletion({ messages, model });
    return explanation;
  } catch (error: unknown) {
    ...
    if (error instanceof Error) {
      ...
      if ("status" in error && typeof (error as { status?: number }).status === "number") {
        ...
        if (status === 401) {
          ...
          errorMessage = "Unable to generate explanation: LLM service is currently unavailable. Please try again later.";
        }
      }
    }
    return errorMessage;
        ...
        setItems((prev) => [
          ...prev,
          {
            ...
            content: [
              {
                type: "input_text",
                text: `Warning: model "${model}" is not in the list of available models returned by the current provider.`,
              },
            ],
          },
        ]);
